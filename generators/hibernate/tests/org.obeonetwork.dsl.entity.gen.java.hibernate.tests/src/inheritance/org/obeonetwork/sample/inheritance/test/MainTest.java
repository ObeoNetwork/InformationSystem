...
public class MainTest extends TestCase {
	...
	protected void setUp() throws Exception {
	    super.setUp();
		mainDao = SampleDaoFactory.getInheritanceMainDao();
	...
	public final void testCreateMain() throws DaoException {
	    ...
	    mainDao.createMain(main);
	
	    // Verify result
	    boolean found = false;
	    for (Main currentMain : mainDao.findAllMains()) {
	...
	public final void testFindAllMains() throws DaoException {
	    ...
	    int before = mainDao.findAllMains().size();
	    Assert.assertTrue("FindAll must return at least 0", before >= 0);
	    
	    // Add two elements
	    Main main1 = getMainExample();
	    mainDao.createMain(main1);
	            
	    Main main2 = getMainExample();
	    mainDao.createMain(main2);
	
	    // Verify result
	    int after = mainDao.findAllMains().size();
	    Assert.assertEquals(
