...
//        	schema.setDefault(defaultValue);
		...
		if (soaProperty instanceof Attribute) {
			// Value Constraints
			addValueConstraintsFromSoaToSwg((Attribute) soaProperty, schema);
		}

		return schema;
	}

	private void addValueConstraintsFromSoaToSwg(final Attribute attribute, final Schema<Object> schema) {
		Objects.requireNonNull(attribute);
		Objects.requireNonNull(schema);

		if (attribute.eIsSet(EnvironmentPackage.Literals.ATTRIBUTE__MINIMUM)) {
			final String minimum = attribute.getMinimum();
			schema.setMinimum(new BigDecimal(minimum));
		}
		if (attribute.eIsSet(EnvironmentPackage.Literals.ATTRIBUTE__MAXIMUM)) {
			final String maximum = attribute.getMaximum();
			schema.setMaximum(new BigDecimal(maximum));
		}
		if (attribute.eIsSet(EnvironmentPackage.Literals.ATTRIBUTE__PATTERN)) {
			final String pattern = attribute.getPattern();
