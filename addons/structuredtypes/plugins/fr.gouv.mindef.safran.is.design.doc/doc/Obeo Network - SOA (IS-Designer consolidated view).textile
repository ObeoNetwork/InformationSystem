...
h1. Obeo Network - SOA (IS-Designer consolidated view)

Le point de vue _SOA (IS-Designer consolidated view)_ fournit une table permettant de faciliter la saisie d'informations sur le stockage physique des DTO.
Les informations sont stockées sous la forme d'annotations.


h2. Création d'une table _DTO Physical Annotations_
 
Pour pouvoir créer une table _DTO Physical Annotations_ il faut activer le point de vue _DTO (IS-Designer consolidated view)_ sur le projet via le menu contextuel _Viewpoints Selection_ sur le projet ou sur le fichier _aird_.

Une fois le point de vue activé, il est possible de créer une nouvelle table sur un objet SOA de type _System_.
A noter qu'à la première activation du point de vue, une table est créée automatiquement pour chaque objet SOA de type _System_ dans le projet. 

!pics/soa/New table.png!
