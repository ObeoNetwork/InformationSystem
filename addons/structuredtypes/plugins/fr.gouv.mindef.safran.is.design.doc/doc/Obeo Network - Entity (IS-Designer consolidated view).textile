...
h1. Obeo Network - Entity (IS-Designer consolidated view)

Le point de vue _Entity (IS-Designer consolidated view)_ fournit une table permettant de faciliter la saisie d'informations sur le stockage physique des entités.
Les informations sont stockées sous la forme d'annotations.


h2. Création d'une table _Entities Physical Names_
 
Pour pouvoir créer une table _Entities Physical Names_ il faut activer le point de vue _Entity (IS-Designer consolidated view)_ sur le projet via le menu contextuel _Viewpoints Selection_ sur le projet ou sur le fichier _aird_.
