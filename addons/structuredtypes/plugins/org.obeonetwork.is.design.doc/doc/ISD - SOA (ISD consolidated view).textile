...
h1. ISD - SOA (ISD consolidated view)

Le point de vue _SOA (ISD consolidated view)_ fournit une table permettant de faciliter la saisie d'informations sur le stockage physique des DTO.
Les informations sont stockées sous la forme d'annotations.


h2. Création d'une table _DTO Physical Annotations_
 
Pour pouvoir créer une table _DTO Physical Annotations_ il faut activer le point de vue _DTO (ISD consolidated view)_ sur le projet via le menu contextuel _Viewpoints Selection_ sur le projet ou sur le fichier _aird_.
