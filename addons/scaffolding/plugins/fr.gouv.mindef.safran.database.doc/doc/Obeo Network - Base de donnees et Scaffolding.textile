...
Note: Si vous rencontrez des problèmes lors de la comparaison d'une base de donnée ayant des liens vers des éléments externes stockés dans des fichiers différents, il est intéressant de lire la documentation sur la résolution du modèle logique qui est utilisé par _EMF Compare_. Pour plus d'information voir la documentation:
...
h4. Génération d'un _ChangeLog_ Liquibase

*Génération d'un _ChangeLog_ de modification*

Il est possible de générer le fichier _ChangeLog_ Liquibase permettant de passer d'une version d'une base de données à une suivante. Pour cela il faut commencer par comparer les 2 modèles de bases de données en s'assurant que le plus récent est en première position (cf. paragraphe précédent). Puis de sélectionner le bouton _Generate Liquibase_ dans la bar de bouton principale.


!pics/LiquibaseGen_CMP_01.png!

Une hierachie de répertoire sera créée contenant les fichiers suivants:

* Liquibase/<Type BBD>/<Nom DBD>/<Date>/run.changelog.xml
* Liquibase/<Type BBD>/<Nom DBD>/run.changelog.xml
* Liquibase/<Type BBD>/<Nom DBD>/liquibase.properties

Les deux premiers fichiers sont identiques. Le premier permet de garder un historique des changements, alors que le deuxième permet seulement d'utiliser plus facilement les commandes _Liquibase_. En effet, celui-ci est stocké dans le même dossier que le fichier _liquibase.properties_. Le fichier _.properties_ n'est pas écrasé à chaque génération, celui-ci est seulement initialisé à la première génération.

*Génération complète*

Il est possible de générer le fichier _ChangeLog_ Liquibase pour générer complètement une base de données. Pour cela il suffit d'utiliser le menu clic droit sur le modèle et sélectionner _IS Designer > Generate Liquibase Changelog_ 
