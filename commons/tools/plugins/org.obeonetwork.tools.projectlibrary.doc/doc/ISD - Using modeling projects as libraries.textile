...
* *Update referencing projects* The modeling projects that have cross-references to the exported project. The selected projects will import the library produced by the current export and the cross-references will be redirected to the library. Details on importing a library are described in the next section.
