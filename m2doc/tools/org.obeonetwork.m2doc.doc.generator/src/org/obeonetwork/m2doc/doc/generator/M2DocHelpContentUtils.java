...
public final class M2DocHelpContentUtils {
    ...
    public static StringBuffer head(boolean isMainPage, String subject, String pluginName) {
		...
		headBuffer.append("      <script src=\"http://html5shim.googlecode.com/svn/trunk/html5.js\"></script>").append(LS);
		...
		if(!isMainPage) {
			...
			headBuffer.append("    <script src=\"../../../../advanced/synchWithToc.js\" type=\"text/javascript\" xml:space=\"preserve\"><!-- --></script>").append(LS);
			headBuffer.append("    <link charset=\"ISO-8859-1\" href=\"../../../../content/org.eclipse.help.webapp/advanced/breadcrumbs.css\" rel=\"stylesheet\" type=\"text/css\"/>").append(LS);
			headBuffer.append("    <script src=\"../../../../content/org.eclipse.help/livehelp.js\" type=\"text/javascript\" xml:space=\"preserve\"> </script>").append(LS);
			headBuffer.append("    <script type=\"text/javascript\">").append(LS);
			headBuffer.append("    if( self == top ){").append(LS);
			headBuffer.append("    var  anchorParam = location.hash.length > 0 ? '\\u0026anchor=' + location.hash.substr(1) : '';").append(LS);
			headBuffer.append("    window.location.replace( \"../../../../index.jsp?topic=%2F" + pluginName + "%2Fdoc%2Fpages%2FTask_services.html\" + anchorParam);").append(LS);
...
//    		{m:for table | db.tables()} table {m:table.name}, {m:endfor}
    ...
    private static StringBuffer computeAttributeTableRow(EClass eClass, EStructuralFeature feature) {
		...
		tableRowBuffer.append("      <td colspan=\"1\" rowspan=\"1\">" + prettyReturnType(feature) + "</td>").append(LS); // Returned type
		tableRowBuffer.append("      <td colspan=\"1\" rowspan=\"1\">" + computeAttributeM2DocSyntax(eClass, feature) + "</td>").append(LS); // M2Doc Syntax
	...
	private static StringBuffer computeServiceTableRow(Method service) {
		...
		tableRowBuffer.append("      <td colspan=\"1\" rowspan=\"1\">" + getPrettyGenericTypename(service.getGenericReturnType(), service.getReturnType()) + "</td>").append(LS); // Returned type
