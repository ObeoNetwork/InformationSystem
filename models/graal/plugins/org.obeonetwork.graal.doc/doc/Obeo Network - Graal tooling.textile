__(c) Obeo 2011, 2016 -  All rights reserved. This program and the accompanying materials are made available under the terms of the Eclipse Public License v1.0__

@Authors@ Stéphane Thibaudeau
@Contact@ stephane.thibaudeau@obeo.fr

h1. Obeo Network - Outillage Graal
...
bq. File > New > Other ... > Graal Model (Catégorie Obeo Designer For IS)
...
!pics/Graal Editor.png!

L'éditeur permet de visualiser le contenu d'un modèle Graal sous forme arborescente. Pour modifier les propriétés d'un élément il faut utiliser la vue _Properties_ (partie basse sur la capture d'écran précédente). Si cette vue n'est pas visible, il est possible de l'ouvrir par un clic droit sur un élément de l'arbre et sélection de _Show Properties View_.

La création de nouveaux éléments se fait dans l'arbre exigences par le menu contextuel _New Child_  du conteneur où l'on souhaite créer l'élément. Ainsi pour créer une nouvelle tâche dans un système, il faut invoquer, depuis le système voulu, le menu :

bq. New Child > Task

Un modèle Graal peut être édité avec cet éditeur arborescent mais ce n'est pas la méthode privilégiée : les éditeurs graphiques (diagrammes) permettent de remplir cette tâche plus efficacement.
...
h3. Ouverture d'une session _Viewpoint_ et activation du point de vue Graal

L'ouverture d'une session _ViewPoint_ sur un modèle Graal se fait de manière classique :
* En double-cliquant sur un fichier _*.aird_ existant
* En faisant un glisser/déposer d'un fichier _*.graal_ sur la vue _Model Content_
* En créant une nouvelle session via le menu @File > New > Local Session@
...
!pics/Graal Viewpoint Selection.png!
...
Pour créer un diagramme d'un certain type, il suffit de sélectionner l'élément du modèle auquel on veut rattacher le diagramme dans la vue _Model Content_ puis, avec un clic droit, de sélectionner le menu :
...
!pics/Graal Graphs on System.png!

Pour ouvrir un diagramme il suffit de double-cliquer sur le diagramme dans la vue _Model Content_. Attention seuls les diagrammes correspondants aux points de vue sélectionnés sur la session sont affichés dans la vue _Model Content_. 
...
!pics/diagrams/tasksgraph/Tasks Graph.png!
...
!pics/diagrams/tasksgraph/Tasks Graph Actors layer.png!

Les outils fournis par la palette sont :

| !pics/diagrams/tasksgraph/Create task.png! | Création d'une tâche. |
| !pics/diagrams/tasksgraph/Create group.png! | Création d'un groupe. |
| !pics/diagrams/tasksgraph/Create system.png! | Création d'un système. |
| !pics/diagrams/tasksgraph/Create use.png! | Création de lien d'utilisation entre tâches, groupes et systèmes. Les liens d'utilisation se font normalement entre tâches, l'outil de création peut néanmoins être utilisé avec un système ou un groupe comme extrémité : dans ce cas une boîte de dialogue s'ouvre pour permettre de choisir la tâche du système ou du groupe sur laquelle le lien d'utilisation sera réellement créé. |
| !pics/diagrams/tasksgraph/Attach actor.png! | Association entre une tâche et un acteur. Cet outil s'utilise lorsque l'acteur voulu n'est pas déjà visible sur le diagramme. Choisir la tâche à laquelle on souhaite associer l'acteur, une boîte de dialogue s'ouvre et permet de sélectionner un acteur. L'acteur apparaît alors sur le diagramme ainsi qu'un lien de rattachement avec la tâche. *Disponible uniquement si le calque _Actors_ est activé* |
| !pics/diagrams/tasksgraph/Attach actor2.png! | Création d'une association entre une tâche et un acteur déjà visible sur le diagramme. *Disponible uniquement si le calque _Actors_ est activé* |
...
!pics/diagrams/grouptasksgraph/Group Tasks Graph.png!
...
| !pics/diagrams/grouptasksgraph/Import existing task.png! | Import d'une tâche existante. Une boîte de dialogue permet de choisir une tâche existante. La tâche est importée dans le groupe actuel et détachée de son conteneur précédent. |
| !pics/diagrams/tasksgraph/Create task.png! | Création d'une nouvelle tâche. |
| !pics/diagrams/tasksgraph/Create group.png! | Création d'un groupe. |
| !pics/diagrams/tasksgraph/Create use.png! | Création de lien d'utilisation entre tâches, groupes. Les liens d'utilisation se font normalement entre tâches, l'outil de création peut néanmoins être utilisé avec un groupe comme extrémité : dans ce cas une boîte de dialogue s'ouvre pour permettre de choisir la tâche du groupe sur laquelle le lien d'utilisation sera réellement créé. |
| !pics/diagrams/tasksgraph/Attach actor.png! | Association entre une tâche et un acteur. Cet outil s'utilise lorsque l'acteur voulu n'est pas déjà visible sur le diagramme. Choisir la tâche à laquelle on souhaite associer l'acteur, une boîte de dialogue s'ouvre et permet de sélectionner un acteur. L'acteur apparaît alors sur le diagramme ainsi qu'un lien de rattachement avec la tâche. *Disponible uniquement si le calque _Actors_ est activé* |
| !pics/diagrams/tasksgraph/Attach actor2.png! | Création d'une association entre une tâche et un acteur déjà visible sur le diagramme. *Disponible uniquement si le calque _Actors_ est activé* |
...
!pics/diagrams/actionsplan/Actions Plan.png!
...
!pics/diagrams/actorsgraph/Actors Graph.png!
...
!pics/diagrams/usecasesmainview/Use Cases Main View.png!
...
!pics/diagrams/usecasediagram/Use Case Diagram.png!
...
!pics/diagrams/domainclasses/namespaces_hierarchy.png!

h4. Eléments affichés et palette d'outils

Les éléments suivants sont affichés sur le diagramme :

* _Namespace_ : chaque package contenu dans le System est affiché. Si unpackage est contenu par un autre package il apparaît à l'intérieur de son package parent. L'icône d'un packae apparaît blanche ou grisée selon si le package est respectivement vide ou non.

Les outils fournis par la palette sont :

| !pics/diagrams/domainclasses/Create namespace.png! | Création d'un package. Un package peut être créé sur le fond du diagramme ou à l'intérieur d'un autre package. |
...
!pics/diagrams/domainclasses/Domain classes diagram.png!
...
| !pics/diagrams/domainclasses/Create namespace.png! | Création d'un sous-package. |
| !pics/diagrams/domainclasses/Create domain class.png! | Création d'une classe participante. |
| !pics/diagrams/domainclasses/Create attribute.png! | Création d'un attribut de classe participante. |
| !pics/diagrams/domainclasses/Create enumeration.png! | Création d'une énumération. |
| !pics/diagrams/domainclasses/Create literal.png! | Création d'une valeur d'énumération. |
| !pics/diagrams/domainclasses/Create relation.png! | Création d'une relation simple. |
| !pics/diagrams/domainclasses/Create composition.png! | Création d'une relation de composition. |
| !pics/diagrams/domainclasses/Create inheritance.png! | Création d'un lien d'héritage. |
| !pics/diagrams/domainclasses/Create bidi relation.png! | Création d'une relation simple bidirectionnelle. |
| !pics/diagrams/domainclasses/Create bidi composition.png! | Création d'une relation de contenance bidirectionnelle. |
| !pics/diagrams/domainclasses/Create external.png! | Ajout d'une classe participante externe. Cet outil permet de faire apparaitre une classe participante définie dans un autre package pour créer des relations avec les classes du package courant. Cet outil est activable via le calque _External Domain Classes_ |
...
Dès qu'un élément Graal est sélectionné sur un diagramme ou dans la vue _Model Content_, les _User Stories_ du système sont affichées dans la vue.
...
!pics/userstories/displayuserstory.png!
