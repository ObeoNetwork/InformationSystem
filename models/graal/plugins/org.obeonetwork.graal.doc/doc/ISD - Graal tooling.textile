...
h6. Association explicite ou implicite d'une tâche

Une _Task_ peut être associée à une _User Story_ de plusieurs manières :

* *Association explicite* : sélectionner la tâche dans un graphe de tâches ou dans le _Model Explorer_, puis cocher la _User Story_ dans la vue _User Stories_. La tâche, les noeuds et les transitions de son plan d'action sont associés. Le contenu des boucles est parcouru récursivement. Les noeuds de type _TaskReference_ sont exclus de cette propagation : les tâches référencées ne sont pas associées automatiquement.
* *Association depuis un noeud du plan d'action* : associer un noeud tel qu'une _UserAction_, une _UserView_, une _AppliAction_, un _AppliEvent_, un opérateur ou un noeud initial, final ou d'abandon associe également automatiquement la tâche qui contient ce noeud. Les autres éléments du plan d'action ne sont pas associés.
* *Association depuis une tâche utilisée* : associer un noeud _TaskReference_ associe la tâche référencée ainsi que son plan d'action. Le noeud _TaskReference_ et la tâche qui le contient ne sont pas associés par cette opération.

L'association d'une transition seule n'entraîne pas l'association automatique de sa tâche contenante.

La dissociation applique les règles réciproques. En particulier, la tâche contenante d'un noeud reste associée tant qu'au moins un autre élément de son plan d'action est encore lié à la _User Story_.
