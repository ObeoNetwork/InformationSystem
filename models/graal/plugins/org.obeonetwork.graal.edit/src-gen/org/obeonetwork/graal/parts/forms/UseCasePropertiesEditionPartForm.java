...
public class UseCasePropertiesEditionPartForm extends SectionPropertiesEditingPart implements IFormPropertiesEditionPart, UseCasePropertiesEditionPart {
	...
	protected Composite createTypesReferencesTable(FormToolkit widgetFactory, Composite parent) {
		this.types = new ReferencesTable(getDescription(GraalViewsRepository.UseCase.Properties.types, GraalMessages.UseCasePropertiesEditionPart_TypesLabel), new ReferencesTableListener	() {
		...
		this.types.setHelpText(propertiesEditionComponent.getHelpContent(GraalViewsRepository.UseCase.Properties.types, GraalViewsRepository.FORM_KIND));
		this.types.createControls(parent, widgetFactory);
		this.types.addSelectionListener(new SelectionAdapter() {
			
			public void widgetSelected(SelectionEvent e) {
				if (e.item != null && e.item.getData() instanceof EObject) {
					propertiesEditionComponent.firePropertiesChanged(new PropertiesEditionEvent(UseCasePropertiesEditionPartForm.this, GraalViewsRepository.UseCase.Properties.types, PropertiesEditionEvent.CHANGE, PropertiesEditionEvent.SELECTION_CHANGED, null, e.item.getData()));
				}
			}
	...
	protected void addTypes() {
		...
		"types", propertiesEditionComponent.getEditingContext().getAdapterFactory(), current.eResource()) {
			...
			public void process(IStructuredSelection selection) {
				for (Iterator<?> iter = selection.iterator(); iter.hasNext();) {
					EObject elem = (EObject) iter.next();
					propertiesEditionComponent.firePropertiesChanged(new PropertiesEditionEvent(UseCasePropertiesEditionPartForm.this, GraalViewsRepository.UseCase.Properties.types,
						PropertiesEditionEvent.COMMIT, PropertiesEditionEvent.ADD, null, elem));
				}
				types.refresh();
			}
	...
	protected void moveTypes(EObject element, int oldIndex, int newIndex) {
		propertiesEditionComponent.firePropertiesChanged(new PropertiesEditionEvent(UseCasePropertiesEditionPartForm.this, GraalViewsRepository.UseCase.Properties.types, PropertiesEditionEvent.COMMIT, PropertiesEditionEvent.MOVE, element, newIndex));
		types.refresh();
	}
	...
	protected void removeFromTypes(EObject element) {
		propertiesEditionComponent.firePropertiesChanged(new PropertiesEditionEvent(UseCasePropertiesEditionPartForm.this, GraalViewsRepository.UseCase.Properties.types, PropertiesEditionEvent.COMMIT, PropertiesEditionEvent.REMOVE, null, element));
		types.refresh();
	}
