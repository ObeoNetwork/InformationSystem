...
public class AppliActionAppliActionPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			// init values
			if (appliAction.getDescription() != null && isAccessible(GraalViewsRepository.AppliAction.Properties.description))
				appliActionPart.setDescription(EcoreUtil.convertToString(EcorePackage.Literals.ESTRING, appliAction.getDescription()));
			if (appliAction.getName() != null && isAccessible(GraalViewsRepository.AppliAction.Properties.name))
			...
			appliActionPart.addFilterToOutgoingTransitions(new ViewerFilter() {
				...
				public boolean select(Viewer viewer, Object parentElement, Object element) {
					if (element instanceof EObject)
						return (!appliActionPart.isContainedInOutgoingTransitionsTable((EObject)element));
			...
			appliActionPart.addFilterToIncomingTransitions(new ViewerFilter() {
				...
				public boolean select(Viewer viewer, Object parentElement, Object element) {
					if (element instanceof EObject)
						return (!appliActionPart.isContainedInIncomingTransitionsTable((EObject)element));
					return element instanceof Resource;
				}
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {
			...
			if (EnvironmentPackage.eINSTANCE.getObeoDSMObject_Description().equals(msg.getFeature()) && appliActionPart != null && isAccessible(GraalViewsRepository.AppliAction.Properties.description)){
			...
			if (GraalPackage.eINSTANCE.getNamedElement_Name().equals(msg.getFeature()) && appliActionPart != null && isAccessible(GraalViewsRepository.AppliAction.Properties.name)) {
				if (msg.getNewValue() != null) {
