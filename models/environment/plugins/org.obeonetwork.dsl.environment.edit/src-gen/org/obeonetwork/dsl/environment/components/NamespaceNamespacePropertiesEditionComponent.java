...
public class NamespaceNamespacePropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			// init values
			if (namespace.getName() != null && isAccessible(EnvironmentViewsRepository.Namespace.Properties.name))
				namespacePart.setName(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, namespace.getName()));
			
			if (namespace.getDescription() != null && isAccessible(EnvironmentViewsRepository.Namespace.Properties.description))
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {
			...
			if (EnvironmentPackage.eINSTANCE.getNamespace_Name().equals(msg.getFeature()) && namespacePart != null && isAccessible(EnvironmentViewsRepository.Namespace.Properties.name)) {
			...
			if (EnvironmentPackage.eINSTANCE.getObeoDSMObject_Description().equals(msg.getFeature()) && namespacePart != null && isAccessible(EnvironmentViewsRepository.Namespace.Properties.description)) {
				if (msg.getNewValue() != null) {
