...
public class ComponentComponentPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			// init values
			if (component.getName() != null && isAccessible(SoaViewsRepository.Component.Properties.name))
				componentPart.setName(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, component.getName()));
			
			if (component.getDescription() != null && isAccessible(SoaViewsRepository.Component.Properties.description))
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {
			...
			if (SoaPackage.eINSTANCE.getComponent_Name().equals(msg.getFeature()) && componentPart != null && isAccessible(SoaViewsRepository.Component.Properties.name)) {
			...
			if (EnvironmentPackage.eINSTANCE.getObeoDSMObject_Description().equals(msg.getFeature()) && componentPart != null && isAccessible(SoaViewsRepository.Component.Properties.description)){
				if (msg.getNewValue() != null) {
			...
			try {
				if (SoaViewsRepository.Component.Properties.name == event.getAffectedEditor()) {
					...
					if (newValue instanceof String) {
						newValue = EcoreUtil.createFromString(SoaPackage.eINSTANCE.getComponent_Name().getEAttributeType(), (String)newValue);
					}
