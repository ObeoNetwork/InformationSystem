...
public class ServiceServicePropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(SoaViewsRepository.Service.Properties.synchronization)) {
				servicePart.initSynchronization((EEnum) SoaPackage.eINSTANCE.getService_Synchronization().getEType(), service.getSynchronization());
			}
			if (isAccessible(SoaViewsRepository.Service.Properties.kind)) {
				servicePart.initKind((EEnum) SoaPackage.eINSTANCE.getService_Kind().getEType(), service.getKind());
			}
			if (service.getName() != null && isAccessible(SoaViewsRepository.Service.Properties.name))
				servicePart.setName(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, service.getName()));
			
			if (service.getDescription() != null && isAccessible(SoaViewsRepository.Service.Properties.description))
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {
			...
			if (SoaPackage.eINSTANCE.getService_Synchronization().equals(msg.getFeature()) && isAccessible(SoaViewsRepository.Service.Properties.synchronization))
				servicePart.setSynchronization((SynchronizationKind)msg.getNewValue());
			
			if (SoaPackage.eINSTANCE.getService_Kind().equals(msg.getFeature()) && isAccessible(SoaViewsRepository.Service.Properties.kind))
				servicePart.setKind((InterfaceKind)msg.getNewValue());
			
			if (SoaPackage.eINSTANCE.getService_Name().equals(msg.getFeature()) && servicePart != null && isAccessible(SoaViewsRepository.Service.Properties.name)) {
			...
			if (EnvironmentPackage.eINSTANCE.getObeoDSMObject_Description().equals(msg.getFeature()) && servicePart != null && isAccessible(SoaViewsRepository.Service.Properties.description)){
				if (msg.getNewValue() != null) {
			...
			try {
				if (SoaViewsRepository.Service.Properties.synchronization == event.getAffectedEditor()) {
					...
					if (newValue instanceof String) {
						newValue = EcoreUtil.createFromString(SoaPackage.eINSTANCE.getService_Synchronization().getEAttributeType(), (String)newValue);
					}
				...
				if (SoaViewsRepository.Service.Properties.kind == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
						newValue = EcoreUtil.createFromString(SoaPackage.eINSTANCE.getService_Kind().getEAttributeType(), (String)newValue);
					}
				...
				if (SoaViewsRepository.Service.Properties.name == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
						newValue = EcoreUtil.createFromString(SoaPackage.eINSTANCE.getService_Name().getEAttributeType(), (String)newValue);
					}
