...
public class CallMessageCallMessagePropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			// init values
			if (callMessage.getName() != null && isAccessible(InteractionViewsRepository.CallMessage.Properties.name))
				callMessagePart.setName(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, callMessage.getName()));
			
			if (callMessage.getDescription() != null && isAccessible(InteractionViewsRepository.CallMessage.Properties.description))
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {
			...
			if (InteractionPackage.eINSTANCE.getNamedElement_Name().equals(msg.getFeature()) && callMessagePart != null && isAccessible(InteractionViewsRepository.CallMessage.Properties.name)) {
			...
			if (EnvironmentPackage.eINSTANCE.getObeoDSMObject_Description().equals(msg.getFeature()) && callMessagePart != null && isAccessible(InteractionViewsRepository.CallMessage.Properties.description)) {
				if (msg.getNewValue() != null) {
