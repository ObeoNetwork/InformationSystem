...
public class ReturnMessageReturnMessagePropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			// init values
			if (returnMessage.getName() != null && isAccessible(InteractionViewsRepository.ReturnMessage.Properties.name))
				returnMessagePart.setName(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, returnMessage.getName()));
			
			if (returnMessage.getDescription() != null && isAccessible(InteractionViewsRepository.ReturnMessage.Properties.description))
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {
			...
			if (InteractionPackage.eINSTANCE.getNamedElement_Name().equals(msg.getFeature()) && returnMessagePart != null && isAccessible(InteractionViewsRepository.ReturnMessage.Properties.name)) {
			...
			if (EnvironmentPackage.eINSTANCE.getObeoDSMObject_Description().equals(msg.getFeature()) && returnMessagePart != null && isAccessible(InteractionViewsRepository.ReturnMessage.Properties.description)) {
				if (msg.getNewValue() != null) {
