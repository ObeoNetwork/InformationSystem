...
define(["require", "app/App", "app/view/ViewUtil", "app/view/widget/StatsHelper", "ember", "raphael" ], function(require) {
	"use strict";
	...
	var App = require("app/App"),
		...
		StatsHelper = require("app/view/widget/StatsHelper");
	// ember does not need to be stored in a variable.
	// raphael does not need to be stored in a variable.
/** End of user code */

	
	var OfficeStats = {},
		_Class = Em.View.extend({ templateName : "stats" });

	
	
	/*
	 * create a OfficeStats widget
	 * @return the OfficeStats widget instance
	 */
	OfficeStats.create = function() {
		ViewUtil.loadWidgetTemplate("./asset/template/widget/other/stats");

		var view = _Class.create({
			tabStats : [],
			
			loadContent : cb_loadStats,
			didInsertElement : function() {App.commonCtrl.loadOfficeStatsInWidget(this);},
	...
	var _draw = function(tabStats, node) {
		...
		data = [];
		var i= 0;
		for(i; i < tabStats.length; i++)
		{
			labels.push(tabStats[i].desc);
			data.push(tabStats[i].res);
		}
		// Draw
		...
		for (var i = 0, ii = labels.length; i < ii; i++) {
			var y = Math.round(height - bottomgutter - Y * data[i]), x = Math.round(leftgutter + X * (i + .5)), t = r.text(x, height - 6, labels[i]).attr(txt).toBack();
			if (!i) {
				p = ["M", x, y, "C", x, y];
			...
			if (i && i < ii - 1) {
				...
				var a = _getAnchors(X0, Y0, x, y, X2, Y2);
			...
			(function(x, y, data, lbl, dot) {
				...
				rect.hover(function() {
					...
					var ppp = r.popup(x, y, label, side, 1), anim = Raphael.animation({
						path : ppp.path, transform : ["t", ppp.dx, ppp.dy]
