...
//	
	public Collection<BindingInfo> getRelatedBindingInfos(Namespace namespace) {
		// Use of LinkedHashSet to keep order and avoid potential lock/unlock loops problems
		...
		// - binding concerning a Structured Type directly from the namespace
		...
		for (Type containedType : namespace.getTypes()) {
			if (containedType instanceof StructuredType) {
				...
				getRelatedBindingInfos(containedStructuredType);
			}
		}
...
//		}
	...
	private Collection<BindingInfo> getRelatedBindingInfos(StructuredType structuredType) {
		Collection<EObject> targets = new EObjectQuery(structuredType).getInverseReferences(EnvironmentPackage.Literals.BINDING_INFO__TARGETS);
