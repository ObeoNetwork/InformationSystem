...
public class ISPropertiesTabsRegistry extends EEFTabbedPropertyRegistry {
	...
	public List<IEEFTabDescriptor> getTabDescriptors(IWorkbenchPart part, ISelection input) {
		...
		sortableDescriptors.sort(new Comparator<IEEFTabDescriptor>() {
			@Override
			public int compare(IEEFTabDescriptor tab1, IEEFTabDescriptor tab2) {
				return getTabOrder(tab1) - getTabOrder(tab2);
			}
		});
	...
	private static final Pattern TAB_ORDER_PATTERN = Pattern.compile(".*\\{isconf:tabOrder=([0-9]+)\\}.*");
