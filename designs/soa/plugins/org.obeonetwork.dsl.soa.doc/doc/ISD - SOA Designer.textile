__Copyright (c) 2008, 2026 Obeo -  All rights reserved. This program and the accompanying materials are made available under the terms of the Eclipse Public License v2.0__

h1(#soa_designer). ISD - SOA Designer
...
h2(#model_creation). Création d'un modèle Soa
...
Lorsqu'un modèle soa est créé à l'aide de ce wizard, les points de vues _SOA Views_, _SOA (ISD Consolidated view)_ et _Environment View_ sont activés.
* Le point de vue _SOA Views_ est décrit dans la section suivante,
* Le point de vue _SOA (ISD Consolidated view)_ est décrit dans la section de documentation "Obeo Network – SOA (ISD consolidated view)":../../org.obeonetwork.is.design.doc/doc/ISD%20-%20SOA%20(ISD%20consolidated%20view).html,
...
h2(#viewpoint). Point de vue _SOA Views_
...
h3(#diagram_handling). Création/ouverture de diagrammes
...
h4(#soa_diagram_tools). Eléments affichés et palette d'outils
...
h4(#soa_diagram_navigation_tools). Navigation vers d'autres diagrammes
...
h3(#component_diagram). Component Contract Diagram
...
h4(#component_diagram_tools). Eléments affichés et palette d'outils
...
h4(#component_diagram_layers). Calques
...
h3(#namespace_hierarchy). DTO Namespaces Hierarchy (pour l'objet racine _Components_)
...
h4(#namespace_hierarchy_description). Eléments affichés et palette d'outils
...
h4(#namespace_navigation). Navigation vers d'autres diagrammes
...
h3(#dto_diagram). Diagramme de DTOs
...
h4(#dto_diagram_tools). Eléments affichés et palette d'outils
...
h2(#rest_api). Exposition des services et modélisation d'API Rest
...
h3(#rest_api_modeling). Modélisation d'API Rest
...
h4(#rest_api_terminology). Terminologie _SOA_ vs terminologie _REST_
...
h4(#open_api). Modélisation d'une API Rest OpenAPI
...
h4(#open_api_tag). Modélisation d'un Tag OpenAPI
...
h4(#open_api_service). Modélisation d'un Service OpenAPI
...
h6(#rest_api_value_constraints). Contraintes sur les valeurs
...
h5(#rest_api_paginated_parameter). Modélisation d'un paramètre de pagination
...
h5(#rest_api_output_fault_parameter). Modélisation d'un paramètre de retour ou d'erreur
...
h5(#rest_api_parameter_media_types). Modélisation des média-types
...
h4(#security). Modélisation de la sécurité

h5(#security_schemes). Modélisation des schémas de sécurité
...
Un _Security Scheme_ de type OAuth2 requière la spécification de _Flows_, ainsi que de leurs _Scopes_ :
...
Open ID Connect est une couche d'identification construite sur le protocole OAuth2, avec quelques restrictions.
A la création d'un Security Scheme de type Open ID Connect, un Flow de type AUTHORIZATIONCODE est automatiquement créé. Ce Flow ne peut pas être supprimé, et d'autres flows ne peuvent pas être créés.
Le Flow AUTHORIZATIONCODE peut être édité dans le seul but d'y ajouter des scopes. Les champs URL sont inactivés :

!pics/securityschemes/Dialog_Flow_Edition_OpenID.png!
...
h5(#security_scheme_usage). Utilisation des schémas de sécurité
...
h4(#property_extension). Modélisation des extensions de propriétés
...
h3(#open_api). Export / Import au format Swagger OpenAPI

h4(#open_api_export). Export au format Swagger OpenAPI
...
h4(#open_api_import). Import d'un fichier de spécification Swagger
...
h3(#open_api_convert). Conversion de fichier Swagger/OpenAPI vers le format OpenAPI 3.1.0
...
h2(#requirements). Gestion des exigences
...
h2(#linked_documentation). Gestion de la documentation liée
...
h2(#interaction_diagram). Création de diagramme d'interaction
...
h2(#state_machine). Création de diagramme de machines à états
