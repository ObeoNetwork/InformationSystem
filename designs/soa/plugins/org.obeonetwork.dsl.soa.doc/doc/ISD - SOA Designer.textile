...
Dans l'explorateur de modèles, tout fichier Json ou Yaml qui est au format OpenAPI 3.0.x est convertible en un nouveau fichier au format OpenAPI 3.1.0 via un menu dédié:

!{width:60%}pics/exposition/Menu_SwaggerConvert.png!

Si la conversion réussit, le nouveau fichier est créé dans l'élément parent du fichier initial. Son nom est le nom du fichier initial suffixé de "-3.1.0" avant l'extension du fichier. Si la conversion échoue, des logs sont disponibles dans la vue Error Log.
